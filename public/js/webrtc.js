// Общие функции для работы с WebRTC
...
// Инициализация WebRTC для репетитора/ученика
function initWebRTC(socket, roomId, role) {
  ...
  // Демонстрация экрана (только для репетитора)
...
function setupSocketListeners(socket) {
  ...
  socket.on('peer-video-toggle', ({ userId, kind, enabled }) => {
    // можно отображать статус, но в этом примере просто логируем
