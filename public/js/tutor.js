...
document.getElementById('lock-btn').addEventListener('click', function() {
  isLocked = !isLocked;
  this.innerHTML = isLocked ? '<i class="fas fa-lock"></i> Доступ закрыт' : '<i class="fas fa-unlock"></i> Доступ открыт';
  this.style.background = isLocked ? 'var(--danger)' : 'var(--success)';
...
socket.on('init-canvas', (data) => {
  ...
  if (data.locked !== undefined) {
    ...
    btn.innerHTML = isLocked ? '<i class="fas fa-lock"></i> Доступ закрыт' : '<i class="fas fa-unlock"></i> Доступ открыт';
    btn.style.background = isLocked ? 'var(--danger)' : 'var(--success)';
  }
});
